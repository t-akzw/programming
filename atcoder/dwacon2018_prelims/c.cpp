...
pair<V,int> waf(int n){
	int last=-1,sum=0;
	V w(n);
	rep(i,n){
    int x;cin >> x;
		if(last==x) w[i]=w[i-1]+1; else w[i]=1;
		last=x;
		sum+=x;
	}
...
int solve(V w,int sum){
	V dp(sum+1,0);
	dp[0]=1;
	rep(i,w.size()) {
    rep2(j,w[i],sum+1) {
      debug("---");
      cout << "i:" << i << ",j: " << j << ",dpj:" << dp[j] << ",dpjw:" << dp[j-w[i]] << ",j-wi:" << j-w[i] << endl;
      add(dp[j],dp[j-w[i]]);
      cout << "i:" << i << ",j: " << j << ",dpj:" << dp[j] << ",dpjw:" << dp[j-w[i]] << ",j-wi:" << j-w[i] << endl;
    }
  }
  rep(i,dp.size()) debug(i);
	return dp[sum];
}
...
void solve() {
  int n,m; cin>>n>>m;
	auto a=waf(n);
	auto b=waf(m);
	int c=solve(a.first,b.second);
	int d=solve(b.first,a.second);
	cout<<c*d%MOD<<endl;
}
// =================================
