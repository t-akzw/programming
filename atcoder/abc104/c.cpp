...
void solve() {
  ...
  int D, G;
  cin >> D >> G;
  rep2(i, 0, D) cin >> P[i] >> C[i];

  G /= 100;
  rep2(i, 0, D) C[i] /= 100;

  rep2(i, 0, D + 1) rep2(point, 0, PMAX) dp[i][point] = INFI;
  dp[0][0] = 0;

  rep2(i, 0, D) rep2(point, 0, PMAX) {
      rep2(j, 0, P[i]) {
        chmin(dp[i + 1][point + (i + 1)*j], dp[i][point] + (int)j);
        //debug("-----");debug(dp[i + 1][point + (i + 1)*j]);debug(dp[i][point] + (int)j);
      }
      chmin(dp[i + 1][point + (i + 1)*P[i] + C[i]], dp[i][point] + P[i]);
      //debug("-----");debug(dp[i + 1][point + (i + 1)*P[i] + C[i]]);debug(dp[i][point] + P[i]);
  }

  rep2(point, G, PMAX) chmin(ans, dp[D][point]);
  cout << ans << endl;
}
// =================================
