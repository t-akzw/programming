...
void dfs(const vector<vector<pair<int, int>>> &G, int v) {
  seen[v] = 1;

  for (auto next : G[v]) {
    int next_v = next.first;
    int dist = next.second;
    //cout << "nv:" << next_v << ",dist:" << dist << ",dpnv:" << dp[next_v] << ",dp+dist:" << (ll)dp[v]+(ll)dist << endl;
    mins(dp[next_v], (ll)dp[v]+(ll)dist);
...
void solve() {
  ll ans = 0;
  int n; cin>>n;
  rep(i,n-1) { int a,b,c; cin>>a>>b>>c; --a; --b; G[a].push_back(make_pair(b,c)); G[b].push_back(make_pair(a,c)); }
  int q,k; cin>>q>>k; --k;
  dp[k] = 0;
  dfs(G, k);
  rep(i,q) {
