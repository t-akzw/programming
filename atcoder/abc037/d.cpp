...
void dfs(int i, int j) {
  ...
  rep(k,4) {
    ...
    if (a[next_i][next_j] <= a[i][j]) continue;
    //cout << "i:" << i << ",j:" << j << ",k:" << k << ",next_i:" << next_i << ",next_j:" << next_j << ",anext:" << a[next_i][next_j] << ",a:" << a[i][j] << ",dpn:" << dp[next_i][next_j] << endl;
    dfs(next_i,next_j);
...
void solve() {
  ...
  rep(i,h)rep(j,w) ans += dp[i][j]; ans %= MOD;
  /*
  rep(i,h) {
    rep(j,w) {
      cout << dp[i][j] << " ";
