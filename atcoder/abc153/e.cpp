...
void solve() {
  ...
  rep(i,H) {
    ...
    rep(j, n) {
      while (tmp < H) {
        debug("===="); debug(v[j].first); debug(v[j].second);
        debug(tmp); debug(dp[tmp]); debug(dp[(tmp-v[j].first)]);
        if (tmp-v[j].first >= 0) mins(dp[tmp], dp[(tmp-v[j].first)]+v[j].second+1);
        debug("xxx"); debug(tmp); debug(dp[tmp]); debug(dp[(tmp-v[j].first)]);
        tmp += v[j].first;
      }
    }
  }
