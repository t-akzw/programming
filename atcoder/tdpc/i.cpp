...
void solve() {
  ...
  for(int i=3;i<=n;i+=3){
    rep(l,n){
      r=l+i;
      if(r>n) break;
      if(s[l]=='i'&&s[r-1]=='i') for(int j=l+1;j<=r-1;j+=3) if (s[j]=='w'&&dp[l+1][j]&&dp[j+1][r-1]) dp[l][r]=1;
      for(int j=l;j<=r;j+=3) if (dp[l][j]&&dp[j][r]) dp[l][r] = 1;
    }
  }
  rep(i,n){
    maxs(d[i+1],d[i]);
    for(int j=i+3;j<=n;j+=3) if(dp[i][j]) maxs(d[j],(d[i]+(j-1)/3));
  }
  ans = d[n];
  cout << ans << endl;
}
