...
void solve() {
  ...
  rep(i,m) { int a,b,c; cin>>a>>b>>c; a--;b--; G[a].pb(edge({b,c})); GR[b].pb(edge({a,c})); }
  dijkstra(0, G, dist);  rep(i,n) tmp[i] += dist[i];
  dijkstra(0, GR, dist); rep(i,n) tmp[i] += dist[i];
  rep(i,n) {
    ll earn = (ll)(t-tmp[i])*(ll)a[i];
    if (t < tmp[i]) continue;
    maxs(ans, earn);
  }
  cout << ans << endl;
