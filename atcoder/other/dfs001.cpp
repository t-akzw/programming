// https://atcoder.jp/contests/atc001/submissions/421696
...
vector<vector<bool>> used(510, vector<bool>(510, false));

bool dfs(int hi, int wj) {
  bool tmp = false;
  if (wj < 0 || w < wj) return false;
  if (hi < 0 || h < hi) return false;
  if (used.at(hi).at(wj)) return false;
  //cout << "hi: " << hi << ", wj: " << wj << ", used: " << used.at(hi).at(wj) << ", now: " << c.at(hi)[wj] << endl;
  used.at(hi).at(wj) = true;
  if ( c.at(hi).substr(wj, 1).compare(gg) == 0 ) return true;
  if (0 < wj) {
    //cout << "wwww hi: " << hi << ", wj: " << wj << ", used: " << used.at(hi).at(wj) << ", now: " << c.at(hi)[wj] << endl;
    // 左側
    if (c.at(hi).substr(wj, 1).compare(xx) != 0  && !used.at(hi).at(wj-1)) {
      //cout << "left" << endl;
      tmp = dfs(hi, wj-1);
      if(tmp) return true;
    }
  ...
  if (0 < hi) {
    //cout << "hhhh hi: " << hi << ", wj: " << wj << ", used: " << used.at(hi).at(wj) << ", now: " << c.at(hi)[wj] << endl;
    // 上側
    if (c.at(hi).substr(wj, 1).compare(xx) != 0 && !used.at(hi-1).at(wj)) {
      //cout << "ue" << endl;
      tmp = dfs(hi-1, wj);
      if(tmp) return true;
    }
...
void solve() {
  ...
  rep(i, h) {
    rep(j, w) {
      //cout << "----" << endl;
      if ( c.at(i).substr(j, 1).compare(ss) == 0 ) {
        res = dfs(i, j);
      }
    }
  }
